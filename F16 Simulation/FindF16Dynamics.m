...
for i=1:length(x_a_lst) 
    ...
    SS_lo = minreal(ss(A_lo,B_lo,C_lo,D_lo));

    SS_lo.OutputName = {'N_pos','E_pos','h','phi','theta','psi','u','alpha','beta','p','q','r','nx','ny','nz','M','q_bar','ps','an'};
    Ss_lo.InputName  = {'T','de','da','dr','flap'};

    % SS_lo.StateName = {'N_pos','E_pos','h','phi','theta','psi','u','alpha','beta','p','q','r','nx','ny','nz','M','q_bar'};
    %% Genrate transfer function for a_n:

    ss_an_de = ss(A_lo,B_lo(:,2),C_lo(19,:),0);

    H_an_de = minreal(tf(ss_an_de));
    
    
    y = lsim(H_an_de,-u,t);  %negative input elevator (nose-up command!)
    

    label = strcat('x_a = ' ,num2str(num2str(x_a/0.3048)), ' ft');
    plot(t,y,'DisplayName',label,'LineWidth',2);
    

end

% plot(t,-u,'DisplayName','Input','Color','black','LineWidth',4)
grid on
...
SS_lat_lo.InputName = {'da','dr','flap'};
